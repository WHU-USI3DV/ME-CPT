...
# Direct
export PYTHONPATH=./
python tools/train.py --config-file configs/s configs/cd/Urb3dcd.py b --options save_path=./results
```
### Testing
The training processing is based on configs in `configs` folder. 
For example:
...
# Direct
export PYTHONPATH=./
python tools/test.py --config-file configs/s configs/cd/Urb3dcd.py b --options save_path=./results
```
