...
class ClsEvaluator(HookBase):
    ...
    def eval(self):
        ...
        self.trainer.comm_info["current_metric_name"] = "m_iou"  # save for saver
...
class SemSegEvaluator(HookBase):
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            loss = output_dict["loss"]
            pred = output.max(1)[1]
            segment = input_dict["segment_cd"]  #cd task
...
@HOOKS.register_module()
class SemSegBiEvaluator(HookBase):
    def after_epoch(self):
        if self.trainer.cfg.evaluate:
            self.eval()
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            intersection, union, target = intersection_and_union_gpu(
                pred,
                segment,
                self.trainer.cfg.data.num_classes-1,
                self.trainer.cfg.data.ignore_index,
            )
            if comm.get_world_size() > 1:
                dist.all_reduce(intersection), dist.all_reduce(union), dist.all_reduce(
        ...
        for i in range(self.trainer.cfg.data.num_classes-1):
            self.trainer.logger.info(
                "Class_{idx}-{name} Result: iou/accuracy {iou:.4f}/{accuracy:.4f}".format(
                    idx=i,
                    name=self.trainer.cfg.data.names[i],
